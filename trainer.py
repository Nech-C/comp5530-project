...
from cpgpo_absDiff import single_CPGPO_absDiff
...
def train_maze_cpgpo_v2_absDiff(num_agents, config):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    # Check and create CPGPO path
    current_time = datetime.now().strftime("%Y%m%d_%H%M%S")
    path_cpgpo = os.path.join(config['path_cpgpo'], current_time)
    config["algo"] = "abs cp diff"
    os.makedirs(path_cpgpo, exist_ok=True)
    ...
    for i in range(num_agents):
        ...
        model = ActorCriticMazeV2().to(device)
        reference_model = random.choice(all_models).to(device)
        trained_model = single_CPGPO_absDiff(env, model, reference_model, config, device)
        model_save_path = os.path.join(path_cpgpo, f"cpgpo_maze_model_{i}.pth")
        torch.save(trained_model.state_dict(), model_save_path)

def test_train_maze_cpgpo_v2_absDiff():
    config = {
        'learning_rate': 0.0007,
        'epoch': 8000,
        'gamma': 0.99,
        ...
        'max_n': 1,
        'epsilon': 0.25,
        'epsilon_decay': 0.001,  # Assuming this represents the decay rate per epoch or condition
        'min_epsilon': 0.0,
        'reference_directory': "./trained_models/maze/v2",  # Directory containing reference models
        'path_cpgpo': "./trained_models/maze/absdiff/trained_n1_cpgpo_models_v1",
    }
    train_maze_cpgpo_v2_absDiff(1, config)
...
if __name__ == "__main__":
    ...
    test_train_maze_cpgpo_v2_absDiff()
